...
/*   Updated: 2025/06/09 20:37:45 by lkloters         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	check_map_walls(t_game *game)
{
	...
	{
		if (game->map_grid[0][j] != '1' || \
			game->map_grid[game->map_height - 1][j] != '1')
			return (false);
		j++;
	}
	i = 1;
	while (i < game->map_height - 1)
	{
		if (game->map_grid[i][0] != '1' || \
			game->map_grid[i][game->map_width - 1] != '1')
			return (false);
		i++;
	}
	return (true);
...
int	validate_map(t_game *game)
{
	if (!is_rectangular(game))
		handle_error(-1, "Map is not rectangular!", game->map_grid);
	if (!check_map_walls(game))
		handle_error(-1, "Map is not surrounded by walls!", game->map_grid);
	if (!check_chars(game))
		handle_error(-1, "Invalid chars!", game->map_grid);
	if (!check_accessibilty(game))
		handle_error(-1, "Collectables/Exit not accessible!", game->map_grid);
	return (0);
}
